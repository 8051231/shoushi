...
public class LeftGestureControl : MonoBehaviour
{
    ...
    void Update()
    {
        if (FirstPersonController.IsInputEnabled == false)
        {
            return;
        }
        ...
        if ((Hi5_Glove_Gesture_Recognition.isLeftFist == 1) && start == false)
        {
            start = true;
            startVec = handTrans.localPosition - bodyTrans.localPosition;
            return;
...
/*
            ...
            {
                transform.parent.Translate(0, 0, Time.deltaTime * movez*(-moveSpeed));
            }
            if (Mathf.Abs(movex) > 0.05f)
            {
                transform.parent.Translate(Time.deltaTime * movex * (-moveSpeed), 0, 0);
            }
